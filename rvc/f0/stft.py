...
class STFT(torch.nn.Module):
    ...
    def forward(
        ...
        if self.use_torch_stft:
            # torch.stft on MPS currently emits repeated resize warnings and can be unstable.
            # Run the kernel on CPU for this path, then move the result back to the original device.
            run_device = input_data.device
            stft_input = input_data
            if input_data.device.type == "mps":
                run_device = torch.device("cpu")
                stft_input = input_data.cpu()

            keyshift_key = str(keyshift) + "_" + str(run_device)
            if keyshift_key not in self.hann_window:
                self.hann_window[keyshift_key] = torch.hann_window(self.win_length).to(
                    run_device
                )
            fft = torch.stft(
                stft_input,
