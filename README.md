...
You can also run the standalone setup script before first launch to download all
missing assets directly from Hugging Face (no additional tools required):

```bash
python3 tools/download_models.py
```

To only validate files without downloading:

```bash
python3 tools/download_models.py --check-only
```

To also download optional assets (e.g. `rmvpe.onnx`, needed on Windows/Linux CPU but
not on macOS/MPS):

```bash
python3 tools/download_models.py --include-optional
```
...
./run.sh
```
To auto-download/check missing assets before launch:
```bash
AUTO_DOWNLOAD_ASSETS=1 ./run.sh
```
...
```bash
git clone https://github.com/mansueli/RVC-WebUI-MacOS.git
cd RVC-WebUI-MacOS
```
...
### Training Reliability (WebUI)

- Training continues even if you close the browser tab.
- Use "Train status" to reconnect and inspect current state, attempts, and recent logs.
- Failures are auto-retried by a supervisor (including OOM-aware batch-size backoff).
- Use "Stop training after current epoch" for a graceful stop.
  - This requests stop at epoch boundary, saves checkpoints, and then exits cleanly.
...
├── requirements/
│   └── python310.txt  # Python 3.10 dependency entrypoint
...
- macOS-focused maintenance and reliability updates for Apple Silicon in this repository
